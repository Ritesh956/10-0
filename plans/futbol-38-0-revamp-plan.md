...
Phases 0-7 are complete, automated-test-verified, and (for Phases 6-7) live-verified against the real API/worker/DB stack. Phases 0-5 closed out the "make the solo game exactly like 38-0" core loop (§17's framing); Phase 6 shipped the Leaderboard; Phase 7 shipped One-Club XI, reusing the Leaderboard's `mode`/`refClubId` infrastructure for per-club boards and comparative trophies rather than building a parallel system. Start **Phase 8 — Daily Challenge mode** next: a deterministic-per-date puzzle (`DailyChallenge` model — date/theme/anchorPlayer/constraints/fixed formation), a `/daily` page reusing the Phase-1 draft components with a live "completion odds" readout (`lib/dailyOdds.ts`), an attempt economy (5 attempts/day + a smaller reroll budget), and a daily leaderboard (reusing Phase 6/7's `mode`/filter infrastructure again — `mode="daily"` is the natural third value). Read §13 in full before starting.
