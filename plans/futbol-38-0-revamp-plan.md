...
## 15. PHASE 10 — Nations Trophy, content pages, and platform polish — ✅ COMPLETE (2026-07-23)

Built as scoped, with the stretch item explicitly deferred as originally called out. **Nations Trophy** followed the One-Club XI (Phase 7) architecture closely rather than inventing a parallel one: `catalog.service.ts` gained a `nationality` filter on both `listClubSeasons` (via `playerSeasons: { some: { player: { nationality } } } }`, scoping the wheel to only clubs with a real eligible player — the same "no dead spin" reasoning as One-Club's `clubId` filter) and `listPlayerSeasons` (filtering a drawn squad down to that nationality), plus a new `listNations()`/`GET /catalog/nations` directory query (`RefPlayer.groupBy` by nationality across the top-5 countries). `DraftContext.lockedNationality` mirrors `lockedClubId`, but as a *single* field — a nationality string doubles as both id and display label, unlike a club's id/name pair. `NationsDirectoryPage` (`/nations`) mirrors `ClubsDirectoryPage` (`/clubs`) exactly. One deliberate divergence from One-Club: **Prime ratings stay available** for Nations-locked drafts (`SetupPage` doesn't force Season-only) — `RefPlayer.nationality` is season-independent, so a career-best substitution can never break the "this really is a player of this nationality" premise the way it would for One-Club's "this really is this club's XI." A new `nations-champion` ("Golden Generation") trophy was added to `TrophyKey` (`packages/domain/src/trophies.ts`, mirrored in `apps/web/src/api/types.ts` and `apps/web/src/lib/trophies.ts` — 3 files, the documented gotcha from Phase 7's own trophy notes), evaluated in `trophy-evaluation.ts`'s existing `evaluateTrophies()` alongside "champions" (same `position === 1` condition, gated on a new `RunSummary.nationsLocked` flag) rather than a separate comparative evaluator like One-Club's club-record trophies — a Nations run's natural benchmark is "won the league," not "beat a prior run's points total." `SeasonsService.finalizeRun` derives `nationsLocked` from `World.settings.nationsNationality` (set once at world-creation, same "never trust the client at read time" pattern as `oneClubClubId`). The leaderboard gained a `nations` mode (`LeaderboardEntry.nationality String?` + `@@index([mode, nationality, points])`, migration `20260722212520_add_leaderboard_nationality`) alongside a `LeaderboardFilters` chip and a `NationsDirectoryPage` deep-link (`?mode=nations&nationality=X`), mirroring `refClubId`'s pattern field-for-field.

**Sign-in gating**: the plan called Nations Trophy "sign-in-gated," but this repo has no existing pattern for excluding *guest* accounts specifically (every `JwtAuthGuard`-protected route, including Daily Challenge's submit endpoint, already accepts a guest's real JWT) — and CLAUDE.md explicitly documents guest-first as a deliberate product decision, not a placeholder to harden later. Nations Trophy was built to require *being signed in* (guest or upgraded) at the same action-level gate (`GuestGateModal` on draft-confirm) every other mode already uses, not a stricter "real account only" gate — earning the trophy still requires a persisted `userId`, which a guest has. Flagging this interpretation explicitly rather than silently narrowing scope.

**Content/SEO pages**: `/how-it-works`, `/how-to-play`, `/best-xi`, `/story` all shipped as new routes, linked from `SiteFooter`. `/best-xi` is our own editorial shortlist (real, factual player/club/position data, same trust level as the rest of the real dataset per `SiteFooter`'s standing disclaimer) rather than anything sourced from a proprietary ranking. **Landing page mode cards**: `LandingPage.tsx`'s `OTHER_MODES` section — found to be rendering One-Club and Daily as `opacity-70` "Not yet" stubs despite both having shipped back in Phases 7-8, stale placeholder copy never updated — was replaced outright with a real `MODE_CARDS` grid (Classic/Multiplayer/One-Club/Daily/Nations, all genuine links), plus a new FAQ section (`<details>` accordion, no prior FAQ scaffold existed anywhere in the app).

**Platform polish**: `index.html` gained a meta description, OG/Twitter tags, `theme-color`, and a `public/manifest.json` + SVG favicon for basic installability. **Not done, deliberately**: per-route/per-mode dynamic meta tags (would need a head-management library with no existing precedent in this codebase) and a full offline-capable service worker — both scoped down to "the cheap wins" rather than a full PWA build-out, since neither was load-bearing for the phase's acceptance criteria. Share-image rendering (`ShareCard.tsx`) was Phase 4/5's own deliverable and untouched this phase — nothing about it needed revisiting. **Stretch item explicitly not built**: "true multi-league European Nights" (pooling Champions League qualifiers across all five leagues) — called out in the plan itself as optional beyond parity; the existing top-8-of-your-division model is unchanged.

**Tests**: `trophy-evaluation.test.ts` (+1, the `nations-champion`/`nationsLocked` boundary), `SetupPage.nations.test.tsx` (new, 2 tests — the nation-lock banner/escape-hatch and confirming Prime stays available, unlike One-Club's forced-Season note). Full monorepo `pnpm typecheck` and `pnpm test` green across all 10 packages (web 127 tests, api 82 tests). Live-verified against the real running stack (API + sim-worker + web, real Neon Postgres): `/nations` correctly lists 136 real nationalities with real player counts from the dataset; the leaderboard's `?mode=nations&nationality=X` deep-link renders the nation filter chip and mode chip correctly; all four content pages and the rebuilt landing page render with zero console errors on a fresh tab.

**A real bug found and fixed in the same session, unrelated to Phase 10's own scope**: a signed-in user could complete a full draft and hit a raw "Unauthorized" on confirm if their 24h JWT had expired — `useAuth().isAuthenticated` only checks whether a token/user is present *locally*, never whether the server still honors it, so `DraftPage.handleConfirmClick`'s `!isAuthenticated` guard doesn't catch a stale-but-present token (a gotcha CLAUDE.md's auth section already flagged as a known risk). Fixed by exporting `ApiError` from `api/client.ts` so `doConfirm()`'s catch block can detect a 401 specifically, clearing the stale session and reopening `GuestGateModal` with a new `reason="session-expired"` variant (distinct copy, plus a "Sign in instead" link to `/signin` — safe because `DraftProvider` sits above the router, so in-progress picks survive the navigation) instead of surfacing the raw error. Regression-tested in `DraftPage.sessionExpiry.test.tsx`.
...
**All 11 phases (0-10) of this revamp plan are now complete**, automated-test-verified (`pnpm typecheck` and `pnpm test` green across all 10 packages: web 127 tests, api 82 tests), and live-verified against the real API/worker/DB stack. Phases 0-5 closed out the "make the solo game exactly like 38-0" core loop (§17's framing); Phase 6 shipped the Leaderboard; Phase 7 shipped One-Club XI, reusing the Leaderboard's `mode`/`refClubId` infrastructure for per-club boards and comparative trophies rather than building a parallel system; Phase 8 shipped the Daily Challenge puzzle mode (its own `DailyChallengeEntry` table rather than folding into `LeaderboardEntry` — see §13); Phase 9a shipped async multiplayer Leagues, genuinely reusing `LeaderboardEntry` for standings this time (joined by worldId, auto-submitted for league worlds); Phase 9b shipped real-time turn-based Live Draft (WebSocket gateway, snake-order turns, timer/auto-pick), deliberately backed by a `MultiplayerLeague` so it hands off to 9a's own simulate/standings pipeline the moment picking finishes rather than building a parallel one — see §14's completion notes for both. Phase 10 shipped Nations Trophy (reusing One-Club's architecture field-for-field, see §15), the content/SEO pages, a real landing-page mode grid (replacing stale "Not yet" stubs left over from before Phases 7-8 shipped), and baseline platform meta/manifest polish — see §15's completion notes, including a real stale-JWT bug found and fixed in the same session (`DraftPage`'s confirm flow now recovers from a 401 instead of surfacing a raw "Unauthorized"). The full plan's feature-parity matrix (§16) is built end to end.

**What's left is explicitly out of this plan's scope, not oversight**: the Phase 10 stretch item ("true multi-league European Nights," pooling Champions League qualifiers across all five leagues instead of the current top-8-of-your-division model); per-route dynamic meta/OG tags and a full offline-capable service worker (§15's "Not done, deliberately"); the ETL era-back-extension to pre-2012 seasons flagged as a standing backlog item since Phase 0 (§5); and the general "Not done" items called out per-phase throughout this doc (submission/report rate-limiting since Phase 6, full one-player-per-slot fillability matching since Phase 7, morale/form progression between matches since the engine-contract section). None of these block using or shipping the app as-is — they're each their own scoped follow-up project if picked up later.
